...
// remove::start[K8S]
String kubernetesRepos = 'https://github.com/davsuapas/DanceSchool-Customer'
// remove::end[K8S]
...
factory.job('jenkins-pipeline-cf-seed') {
	...
	wrappers {
		...
		parameters {
			...
			stringParam('REPO_WITH_BINARIES', 'http://172.19.0.2:8081/artifactory/libs-release-local', "Address to hosted JARs for downloading")
			stringParam('REPO_WITH_BINARIES_FOR_UPLOAD', 'http://172.19.0.2:8081/artifactory/libs-release-local', "Address to hosted JARs for uploading")
...
factory.job('jenkins-pipeline-cf-declarative-seed') {
	...
	wrappers {
		parameters {
			...
			stringParam('M2_SETTINGS_REPO_ID', 'artifactory-local', "Name of the server ID in Maven's settings.xml")
			stringParam('REPO_WITH_BINARIES', 'http://172.19.0.2:8081/artifactory/libs-release-local', "Address to hosted JARs for downloading")
			stringParam('REPO_WITH_BINARIES_FOR_UPLOAD', 'http://172.19.0.2:8081/artifactory/libs-release-local', "Address to hosted JARs")
			stringParam('REPO_WITH_BINARIES_CREDENTIAL_ID', 'repo-with-binaries', "Credential ID of repo with binaries")
			stringParam('GIT_EMAIL', 'email@example.com', "Email used to tag the repo")
			stringParam('GIT_NAME', 'Pivo Tal', "Name used to tag the repo")
			stringParam('SCRIPTS_URL', 'https://github.com/CloudPipelines/scripts/archive/master.tar.gz', "The URL to tarball or URL to git repository containing pipeline functions repository. Has to end either with .tar.gz or .git ")
...
factory.job('jenkins-pipeline-k8s-seed') {
	...
	wrappers {
		parameters {
			...
			stringParam('REPO_WITH_BINARIES_FOR_UPLOAD', 'http://172.19.0.2:8081/artifactory/libs-release-local', "Address to hosted JARs")
			stringParam('REPO_WITH_BINARIES_CREDENTIAL_ID', 'repo-with-binaries', "Credential ID of repo with binaries")
			stringParam('GIT_EMAIL', 'danceschoolmicroservices@gmail.com', "Email used to tag the repo")
			stringParam('GIT_NAME', 'DanceSchool Microservices', "Name used to tag the repo")
			stringParam('SCRIPTS_URL', 'https://github.com/davsuapas/scripts', "The URL to tarball or URL to git repository containing pipeline functions repository. Has to end either with .tar.gz or .git ")
			stringParam('SCRIPTS_BRANCH', 'test_addicional_script', "The branch with pipeline functions")
			stringParam('JENKINS_SCRIPTS_URL', 'https://github.com/CloudPipelines/jenkins.git', "The URL to git repository containing Jenkins setup")
			stringParam('JENKINS_SCRIPTS_BRANCH', 'master', "The branch with Jenkins setup")
			booleanParam('AUTO_DEPLOY_TO_STAGE', false, 'Should deployment to stage be automatic')
			booleanParam('AUTO_DEPLOY_TO_PROD', false, 'Should deployment to prod be automatic')
			booleanParam('API_COMPATIBILITY_STEP_REQUIRED', false, 'Should api compatibility step be present')
			booleanParam('DB_ROLLBACK_STEP_REQUIRED', true, 'Should DB rollback step be present')
			booleanParam('DEPLOY_TO_STAGE_STEP_REQUIRED', true, 'Should deploy to stage step be present')
			...
			stringParam('DOCKER_EMAIL', 'danceschoolmicroservices@gmail.com', "Email used to connect to Docker registry")
			stringParam('DOCKER_REGISTRY_ORGANIZATION', 'danceschool', 'URL to Kubernetes cluster for test env')
